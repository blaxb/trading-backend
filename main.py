...
def submit_alert(alert: AlertIn):
    try:
        ...
        # Look up user
        r = requests.get(f"{SUPABASE_URL}/rest/v1/users?email=eq.{alert.email}", headers=headers)
        users = r.json()
        ...
        if users:
            ...
            # Create user
            r = requests.post(f"{SUPABASE_URL}/rest/v1/users", headers=headers, json={"email": alert.email})
            user_id = r.json()[0]["id"]
        ...
        r = requests.post(f"{SUPABASE_URL}/rest/v1/alerts", headers=headers, json=payload)
